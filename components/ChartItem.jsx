...
const ChartItem = ({ song, isExpanded, onToggleExpand }) => {
  if (!song) return null;
  ...
  const cardClasses = `
    relative bg-gray-900 rounded-lg transition-all duration-300 ease-in-out overflow-hidden border
    ${song.trendStatus === 'rising' ? 'border-amber-400/50 animate-glowing-border' : 'border-gray-700'}
  `;
  ...
  return (
    <div className={cardClasses}>
      ...
      <div 
        className="flex items-center p-4 cursor-pointer hover:bg-gray-800/50"
        onClick={onToggleExpand}
      >
        <div className="flex items-center justify-start w-20">
          <span className={`font-bold ${rankFontSize} ${rankColor} w-8 text-center`}>
            {song.rank}
          </span>
          <div className="w-8 flex justify-center">
            <RankChangeIndicator change={song.rankChange || 0} />
          </div>
        </div>
        
        <div className="flex-grow mx-2 overflow-hidden">
          <p className="text-white font-semibold truncate">{song.title || 'Unknown Title'}</p>
          <p className="text-gray-400 text-sm truncate">{song.artist || 'Unknown Artist'}</p>
          <div className="mt-1 flex items-center space-x-4 text-xs text-gray-500">
            <span>리퍼블릭 지수: {song.republicIndex || 0}</span>
            {song.isHot && <span className="text-amber-400">🔥 급상승</span>}
          </div>
        </div>
        ...
        <div className={`bg-black/50 p-4 border-t border-gray-800 transition-opacity duration-300 ease-in-out ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
          <div className="flex justify-between items-center mb-2">
            ...
            <a 
              ...
              className="bg-red-600 hover:bg-red-700 text-white font-semibold text-xs p-2 rounded-lg transition-colors duration-200 flex items-center gap-2"
            >
          ...
          <div className="h-40">
            {isExpanded && song.rankHistory && <RankChart data={song.rankHistory} />}
          </div>
        </div>
      </div>
    </div>
